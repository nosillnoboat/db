...
nice *create*.rb migrations you eventually get *drop*.rb, *rename*.rb, *add*.rb, etc. migrations mixed with your
*create*.rb migrations (think of this as migration fragmentation). This can also make your table schemas messy
to read depending on how many changes you have applied to a table over time. Instead of being stuck with this
clutter, the remigration process allows you to clean up previous migration fragments and consolidate them into
create migrations instead so that you have a directory of *create*.rb migration files only and better looking
database schemas to boot.

To remigrate a database:

1. Run: db -M -s
2. Remove excess migrations by manually applying changes to existing "create" migrations.
3. Run: db -M -g
4. Edit the "remigrate_generator" reorder the migrations (if necessary, otherwise move to the next step).
5. Run: db -M -e
6. Run: db -M -c
